...
```{r Fig_S2A,fig.width=4.5,fig.height=3.5}
...
```{r Fig_S2B,fig.width=4.5,fig.height=3.5}
...
```{r Fig_S2C,fig.width=7,fig.height=4.5, cache=FALSE}
...
```{r Fig_S2D, fig.width=7,fig.height=4}
#library(lme4)
library(gtools)
...
```{r Fig_S3A,fig.width=4.5,fig.height=4, cache=FALSE}
for (nn in grep('PBMC',names(pbmc),value=TRUE)) {
  DefaultAssay(pbmc[[nn]]) <- 'RNA'
}
...
```{r Fig_S3D,fig.width=7,fig.height=3}
pl <- RNA %>% 
...
```{r Fig_S3B,fig.width=5,fig.height=4}
RNA <- do.call(rbind,
               lapply(pbmc[grepl('PBMC',names(pbmc))], 
                      function(sobj) FetchData(sobj, c('ITGA4','ITGB7','orig.ident')) %>%
                        tibble::rownames_to_column('cell') %>%
...
```{r Fig_S3C,fig.width=3.75,fig.height=4, cache=FALSE}
for (nn in grep('PBMC',names(pbmc),value=TRUE)) {
  DefaultAssay(pbmc[[nn]]) <- 'orig_ADT'
}
...
```{r Fig_S4B,fig.width=4.5,fig.height=3.5, cache=FALSE}
...
```{r Fig_S4C,fig.width=6,fig.height=2}
cols <- c('sample','orig.ident','predicted.celltype.l2')
...
- ITGA4, ITGB7 or ITGB1 positive
- Integrin-b7 positive (after determining a threshold)
- GPR15,  CCR9 or ITGAE positive 
- ITGA4, ITGB7 or ITGB1 negative
- Integrin-b7 negative 


```{r Fig_S4E_S4H_S5C_S5D,fig.width=2.5,fig.height=2.5, cache=FALSE}
library(Binarize)
for (nn in grep('PBMC',names(pbmc),value=TRUE)) {
  DefaultAssay(pbmc[[nn]]) <- 'RNA'
}

cols <- c('sample','orig.ident','predicted.celltype.l2','ITGA4','ITGB7','ITGB1','Integrin-b7','GPR15','CCR9','ITGAE','MKI67')
...
for (ct in c('Mmry CD4','Mmry CD8')) {
  bcs <- list(ITGA4=pbmc_md$cell[grepl(ct,pbmc_md$celltype) & (pbmc_md$ITGA4 > 0)],
              ...
              ITGB1=pbmc_md$cell[grepl(ct,pbmc_md$celltype) & (pbmc_md$ITGB1 > 0)],
              ITGA4neg=pbmc_md$cell[grepl(ct,pbmc_md$celltype) & (pbmc_md$ITGA4==0)],
              ITGB7neg=pbmc_md$cell[grepl(ct,pbmc_md$celltype) & (pbmc_md$ITGB7==0)],
              ITGB1neg=pbmc_md$cell[grepl(ct,pbmc_md$celltype) & (pbmc_md$ITGB1==0)],
              Integrin_b7_neg=pbmc_md$cell[grepl(ct,pbmc_md$celltype) & (pbmc_md$Integrinb7=='neg')])
  for (nn in names(bcs)) {
    if (length(bcs[[nn]]) > 0) {
      tcrcomb <- lapply(tcr_combined,
...
for (ss in c('GPR15','Integrin_b7','ITGA4','ITGAE','ITGB7','ITGB1','ITGA4neg','ITGB7neg','ITGB1neg','Integrin_b7_neg')) {
  for (ct in c('Mmry CD4','Mmry CD8')) {
    pl <- ggplot(df %>% 
                   dplyr::filter(Diagnosis=='UC') %>%
...
```{r Fig_S4H,fig.width=2.5,fig.height=2}
ADT %>% 
...
```{r Fig_S4G, fig.width=2.5,fig.height=2.}
pbmc_frac %>%
  dplyr::filter(Timepoint %in% c('Vedo_0d','Vedo_6wk')) %>%
  dplyr::group_by(celltype,Patient.ID) %>%
  dplyr::mutate(ns=n()) %>%
...
```{r Fig_S5E, fig.width=2.5,fig.height=2.}
pbmc_frac %>%
  dplyr::filter(Timepoint %in% c('Vedo_0d','Vedo_6wk')) %>%
  dplyr::group_by(celltype,Patient.ID) %>%
  dplyr::mutate(ns=n()) %>%
...
```{r Fig_S4D,fig.width=3,fig.height=2.5}
cols <- c('sample','orig.ident','predicted.celltype.l2')
pbmc_md <- do.call(rbind,lapply(pbmc[grepl('PBMC',names(pbmc))], function(x) FetchData(x,cols))) %>%
  tibble::rownames_to_column('cell') %>%
...
```{r Fig_S4D_S5B,fig.width=4,fig.height=4}
dfs <- list()
for (ct in c('CD4 TCM','CD4 TEM',
             'CD8 TCM','CD8 TEM')) {
...
```{r Fig_S5F,fig.width=4.5,fig.height=3.5,cache=FALSE}
...
```{r Fig_S5G,fig.width=4.5,fig.height=3}
dfs <- list()
...
```{r Fig_S5H, fig.width=4.5,fig.height=2.}
pbmc_frac %>%
  dplyr::filter(Timepoint %in% c('Vedo_0d','Vedo_6wk')) %>%
  dplyr::group_by(celltype,Patient.ID) %>%
  dplyr::mutate(ns=n()) %>%
...
```{r Fig_2A,fig.width=4,fig.height=3}
...
```{r Fig_2B,fig.width=4,fig.height=3,cache=FALSE}
...
```{r CD4_umap_material,fig.width=4,fig.height=3}
...
```{r CD4_umap_patient,fig.width=4,fig.height=3}
DimPlot(CD4.integrated,group.by='Patient.ID') +
  theme_void() +
  theme(legend.key.size=unit(3,'mm')) +
  labs(title='')
```
...
```{r CD4_cluster_composition,fig.width=4,fig.height=3}
CD4.integrated[['cluster']] <- Idents(CD4.integrated)
for (col in c('Diagnosis','Patient.ID','Material')) {
  CD4.integrated$tmp <- CD4.integrated@meta.data[,col]
  pl <- rbind(CD4.integrated@meta.data %>% 
                dplyr::group_by(cluster,tmp) %>%
                dplyr::summarise(n=n()) %>%
                dplyr::mutate(frac=n/sum(n),
                              group='by cluster'),
              CD4.integrated@meta.data %>% 
                dplyr::group_by(tmp) %>%
                dplyr::summarise(n=n()) %>%
                dplyr::mutate(frac=n/sum(n),
                              group='all',
                              cluster='all')) %>%
    dplyr::mutate(cluster=factor(cluster,levels=c('all',seq(1,12)))) %>%
    ggplot(aes(x=cluster,y=frac,fill=tmp)) +
    ...
    labs(x='',y='fraction cells',fill='')
  print(pl)
}
```


```{r CD4_markers,fig.width=14,fig.height=4.5,cache=FALSE}
DefaultAssay(CD4.integrated) <- 'RNA'
...
DotPlot(CD4.integrated, features = unique(top10$gene)) +
  theme_classic() +
  theme(axis.text.x=element_text(angle=90,hjust=1,vjust=.5),
        legend.key.size=unit(3,'mm')) +
  labs(x='',y='cluster')
...
```{r Fig_2D, fig.width=2,fig.height=1,cache=FALSE}
library(gtools)
CD4.integrated[['cluster']] <- Idents(CD4.integrated)
...
```{r Fig_2C, fig.width=3,fig.height=2,cache=FALSE}
library(gtools)
...
```{r CD4_regression_DE,cache=FALSE}
DefaultAssay(CD4.integrated) <- 'RNA'
CD4.integrated$cluster1 <- factor(ifelse(Idents(CD4.integrated)=='1','1','other'),levels=c('1','other'))

genes <- list('activation'=c('HLA-DRA','CD40LG','CD38','CD69','CTLA4','AREG','ICOS','LAG3','MKI67','PDCD1','SELL','SLAMF6','HAVCR2','TIGIT'),
              'homing'=c('CXCR3','CXCR5','CCR2','CCR4','CCR5','CCR6','CCR7','CCR9','GPR15','ITGA4','ITGB1','ITGB7'),
              'cytokines'=c('CSF2','IFNG','IL2','IL4','IL5','IL9','IL10','IL17A','IL22','TNF','GZMA','GZMB'),
              'receptors'=c('IL1R1','IL1RAP','IL2RA','IL4R','IL7R','IL12RB2','IL18RAP','IL18R1','IL23R','IFNGR1','IFNGR2','TNFRSF1A','TNFRSF1B','TNFRSF14'),
              'TFs'=c('AHR','BATF','BCL6','CREM','EOMES','FOSB','GATA3','IKZF2','JUN','JUNB','MAF','NFATC1','PRDM1','RORA','RORC','TCF7','TOX','ZBTB7B','TBX21'))

if (!file.exists(file.path(path,'seurat','CD4_cluster1_markers.csv'))) { 
  CD4_cluster1_markers <- FindMarkers(CD4.integrated,test.use='LR',logfc.threshold=0,
                                      latent.vars=c('Material'),min.pct=0.0,
                                      group.by='cluster1',ident.1='1',ident.2='other')
#                                      features=intersect(rownames(CD4.integrated),unname(unlist(genes))))
  write.csv(CD4_cluster1_markers,file.path(path,'seurat','CD4_cluster1_markers.csv')) 
} else {
  CD4_cluster1_markers <- read.csv(file.path(path,'seurat','CD4_cluster1_markers.csv'),row.names=1)
}
```

as scatter plot

```{r Fig_2E,fig.width=4,fig.height=3.5}
top_genes <- CD4_cluster1_markers %>% 
  tibble::rownames_to_column('gene') %>%
  dplyr::slice_max(abs(avg_log2FC),n=30) %>%
  dplyr::pull(gene)
top_genes <- setdiff(c(top_genes,c('ITGA4','ITGB7','CD38','MKI67','CXCR3','CCR7')),c('C12orf75','FLNA','HLA−DRB5','CD99','YWHAQ','HLA−DPB1'))
ggplot(CD4_cluster1_markers,
  aes(x=pct.2,y=pct.1)) +
  geom_abline(slope=1,intercept=0,size=.5,color='gray') +
  #geom_point(size=1,pch=20,alpha=.5,color='gray') +
  geom_point(alpha=.5,size=1,color='gray30') +
  geom_label_repel(data=CD4_cluster1_markers[top_genes,] %>%
                     tibble::rownames_to_column('gene'),
                  aes(label=gene), force=5,max.overlaps=25,
                  label.padding=unit(.5,'mm'),
                  label.size=0,
                  color='black',
                  size=2.5,segment.alpha=.5,segment.size=.5) +
  scale_color_gradient2(low='blue3',mid='gray',high='red3') +
  labs(x='% positive cells other clusters',
       y='% positive cells cluster 1',
       fill='log2 fold change',
       size='p-value') +
  theme_classic() +
  #coord_cartesian(xlim=c(-.1,1),ylim=c(-.1,1)) +
  #guides(color=guide_legend(override.aes=list(shape=15,size=3,label="o"))) +
  scale_shape_manual(values=seq(21,26)) +
  theme(legend.key.size=unit(3,'mm'))
```

as lollipop chart

```{r Fig_2F,fig.width=10,fig.height=3}
CD4_cluster1_markers %>% 
  tibble::rownames_to_column('gene') %>%
  dplyr::inner_join(data.frame(gene=unlist(genes)) %>% tibble::rownames_to_column('group') %>% dplyr::mutate(group=gsub('[0-9]*$','',group)),
                    by='gene') %>%
  dplyr::mutate(group=factor(group),
                gene=factor(gene,levels=unname(unlist(genes)))) %>%
  dplyr::arrange(group,avg_log2FC) %>%
  ggplot(aes(x=gene,y=avg_log2FC,color=avg_log2FC)) +
  geom_segment(aes(xend=gene,yend=0)) +
  geom_point(aes(size=-log10(p_val+1.e-30)),pch=19) +
  scale_color_gradient2(low='blue3',high='red3') +
  labs(y='avg. log2 fold change',
       x='',
       fill='log2 fold change',
       size='p-value') +
  facet_grid(.~group,scales='free_x',space='free_x') +
  theme_classic() +
  coord_cartesian(clip='off') +
  scale_size_continuous(limits=c(0,50),breaks=c(10,20,30),labels=c('1.e-10','1.e-20','<1.e-30')) +
  theme(legend.key.size=unit(3,'mm'),
...
```{r Fig_S8D,fig.width=4,fig.height=3.5,cache=FALSE}
DefaultAssay(CD4.integrated) <- 'orig_ADT'
DotPlot(CD4.integrated, features = c('PD-1','CD25','CD45RA','CD127','CD38','CD103','CD4','HLA-DR'),
        group.by='cluster1', scale=TRUE) + 
...
```{r CD4_clonotypes,fig.width=3,fig.height=2.5,cache=FALSE}
CD4.integrated <- RenameCells(CD4.integrated, add.cell.id=CD4.integrated$orig.ident)

tmp <- lapply(tcr_combined, function(x) x[x$barcode %in% Cells(CD4.integrated),])
CD4.integrated <- combineExpression(tmp, CD4.integrated,  cloneCall="aa")
CD4.integrated$cloneType <- factor(tolower(gsub('Single','Small',gsub(' .*','',CD4.integrated$cloneType))),
                                   levels=c('small','medium','large','hyperexpanded'))

CD4_md <- CD4.integrated@meta.data %>%
  tibble::rownames_to_column('cell') 

dfs <- list()
for (cl in unique(CD4.integrated$cluster)) {
  bcs <- CD4_md$cell[CD4_md$cluster==cl]
  tcrcomb <- lapply(tcr_combined,
                    function(x) x[(x$barcode %in% bcs) & !is.na(x$TCR1) & !is.na(x$TCR2),])
  dfs[[cl]] <- clonalDiversity(tcrcomb,cloneCall='gene',group='sample',exportTable=TRUE,skip.boots=TRUE) %>%
...
ggplot(df %>% 
         #dplyr::filter(Diagnosis=='UC') %>%
         dplyr::mutate(cluster=factor(cluster,levels=as.character(seq(0,12)))), 
       aes(x=cluster,y=`Inv.Simpson`)) +
  geom_boxplot(aes(fill=cluster),outlier.shape=NA) +
  geom_point(width=.25,size=1,color='gray30') +
  theme_classic() +
  coord_cartesian(clip='off') +
  scale_y_log10() +
  labs(x='cluster',y='Inv. Simpson',title='') +
  theme(legend.key.size=unit(4,'mm'),
        legend.box='horizontal',
        legend.position='none',
        axis.text.x=element_text(angle=90,hjust=1,vjust=.5),
...
<!-- ```{r tmp} -->
<!-- tmp <- CD4_md %>% dplyr::filter(!is.na(CTaa)) %>% dplyr::select(cluster,sample) %>% table() %>% as.data.frame()  -->
<!-- dplyr::inner_join(df,tmp,by=c('cluster','sample')) %>%  -->
<!--   ggplot(aes(x=Freq,y=`Inv.Pielou`,label=cluster,color=cluster=='1')) +  -->
<!--   geom_text() -->
<!-- ``` -->
```{r sessionInfo}
sessionInfo()
