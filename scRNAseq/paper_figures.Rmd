...
for (ss in c('GPR15','Integrin_b7','ITGA4','ITGAE','ITGB7')) {
  for (ct in c('Mmry CD4','Mmry CD8')) {
    pl <- ggplot(df %>% 
                   dplyr::filter(Diagnosis=='UC') %>%
...
```{r Fig_2a,fig.width=4.5,fig.height=3.5}
...
```{r Fig_2F_alt1,fig.width=6,fig.height=4}
tmp <- CD4_cluster1_markers %>% 
  tibble::rownames_to_column('gene') %>%
...
ggplot(tmp %>% dplyr::filter(p_val_adj < .01), 
  aes(x=1.e-4+pct.2,y=1.e-4+pct.1,fill=avg_log2FC)) +
  geom_abline(slope=1,intercept=0,size=.5,color='gray') +
  #geom_point(size=1,pch=20,alpha=.5,color='gray') +
  geom_point(aes(shape=group),alpha=.5,size=2) +
  geom_text_repel(aes(label=gene), force=5,max.overlaps=25,
                  size=2.5,segment.alpha=.5,segment.size=.5) + #,nudge_x=.0,nudge_y=.1) +
...
```{r Fig_2F_alt2,fig.width=10,fig.height=3}
ggplot(tmp %>% dplyr::arrange(group,avg_log2FC), aes(x=gene,y=avg_log2FC,color=avg_log2FC)) +
  geom_segment(aes(xend=gene,yend=0)) +
  geom_point(data=tmp,aes(size=-log10(p_val+1.e-30)),pch=19) +
